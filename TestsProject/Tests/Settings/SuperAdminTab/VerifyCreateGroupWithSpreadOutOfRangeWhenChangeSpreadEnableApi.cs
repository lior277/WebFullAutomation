...
namespace TestsProject.Tests.Settings.SuperAdminTab
{
    [NonParallelizable]
    [TestFixture]
    public class VerifyCreateGroupWithSpreadOutOfRangeWhenChangeSpreadEnableApi : TestSuitBase
    {
        ...
        public void VerifyCreateGroupWithSpreadOutOfRangeWhenChangeSpreadEnableApiTest()
        {
            var expectedCreateGroupGeneralSpreadMessage = "default_attr spread is out of range,(0% - 0%)";
            ...
            var actualCreateGroupGeneralSpreadMessage = _apiFactory
            ...
            // create group indices's with out of limit spread
            var actualCreateGroupIndiciesSpreadMessage = _apiFactory
                .ChangeContext<ITradeGroupApi>()
                .PostCreateTradeGroupRequest(_crmUrl, 
                new List<object> { _cryptoGroupIndiciesAttr },  _cryptoGroupName,
                apiKey: _currentUserApiKey, false);

            Assert.Multiple(() =>
            {
                Assert.True(actualCreateGroupGeneralSpreadMessage == expectedCreateGroupGeneralSpreadMessage,
                    $" expected Create Group General Spread Message : {expectedCreateGroupGeneralSpreadMessage}" +
                    $" actual Create Group General Spread Message : {actualCreateGroupGeneralSpreadMessage}");

                Assert.True(actualCreateGroupAppleAssetSpreadMessage == (expectedCreateGroupAppleAssetSpreadMessage),
                    $" expected Create Group Cash Spread Message : {expectedCreateGroupAppleAssetSpreadMessage}" +
                    $" actual Create Group Cash Spread Message : {actualCreateGroupAppleAssetSpreadMessage}");

                Assert.True(actualCreateGroupIndiciesSpreadMessage == (expectedCreateGroupIndiciesSpreadMessage),
                    $" expected Create Group Asset Spread Message : {expectedCreateGroupIndiciesSpreadMessage}" +
                    $" actual Create Group Asset Spread Message : {actualCreateGroupIndiciesSpreadMessage}");
            });
        }
    }
}