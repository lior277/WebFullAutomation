...
{
    [TestFixture]
    public class VerifyClientProfilePermissionApi : TestSuitBase
    {
        ...
        public void VerifyClientProfilePermissionApiTest()
        {
            var expectedErrorMessage = "Method Not Allowed";

            var actualdErrorMessage = _apiFactory
                .ChangeContext<IInformationTabApi>()
                .PutInformationTabRequest(_crmUrl,
                _informationTabData, _currentUserApiKey, false)
                .Message;

            Assert.Multiple(() =>
            {
                Assert.True(actualdErrorMessage == expectedErrorMessage,
                    $" expected Client Profile Permissions: {expectedErrorMessage}" +
                    $" actual Client Profile Permissions: {actualdErrorMessage}");
            });
        }
    }
}